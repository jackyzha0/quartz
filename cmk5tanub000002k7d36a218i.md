...
**Example:** If \\(X\\) is the number of hours you work and your pay is \\(g(X) = 15X + 50\\), you can calculate your expected pay directly from the distribution of your hours.
...
| --- | --- | --- |
| **Distribution** | PMF: $p\_X(x)$ | PDF: $f\_X(x)$ |
| **Expectation** | $\\sum x \\cdot p\_X(x)$ | $\\int x \\cdot f\_X(x) , dx$ |
| **Variance** | $\\sum (x-\\mu)^2 \\cdot p\_X(x)$ | $\\int (x-\\mu)^2 \\cdot f\_X(x) , dx$ |
| **Independence** | $p\_{X,Y} = p\_X \\cdot p\_Y$ | $f\_{X,Y} = f\_X \\cdot f\_Y$ |
