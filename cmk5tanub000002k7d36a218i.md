---
title: "From Coin Flips to High-Dimensions: The Engineer’s Guide to Random Variables"
datePublished: Thu Jan 08 2026 18:59:54 GMT+0000 (Coordinated Universal Time)
cuid: cmk5tanub000002k7d36a218i
slug: from-coin-flips-to-high-dimensions-the-engineers-guide-to-random-variables

---

Probability is the engine behind modern AI, data science, and signal processing. Whether you are dealing with discrete events (like packet loss) or continuous signals (like voltage), understanding how to manipulate random variables is a superpower.

Here is a comprehensive guide to the definitions, analogies, and formulas you need to know.
...
For a discrete random variable $X$ with probability mass function (PMF) $p\_X(x)$:
...
$$E\[Y\] = E\[g(X)\] = \\sum\_x g(x) \\cdot p\_X(x)$$
...
$$p\_X(x) = \\sum\_y p\_{X,Y}(x, y)$$

Linearity of Expectation:

This is one of the most powerful properties in probability. It holds even if variables are dependent.
...
For continuous variables (time, distance, temperature), the probability of being exactly equal to a specific number is 0. Instead, we measure probability over intervals using a **Probability Density Function (PDF)**, $f\_X(x)$.
...
1\. Uniform Distribution ($X \\sim \\text{Uni}(a,b)$)

Every interval of the same length is equally likely.

$$f\_X(x) = \\frac{1}{b-a} \\quad \\text{for } a &lt; x &lt; b$$

2\. Exponential Distribution ($X \\sim \\text{Exp}(\\lambda)$)

Models waiting times (e.g., time until the next server request). It has the unique Memoryless Property:

$$P(X &gt; s + t | X &gt; t) = P(X &gt; s)$$

If you've waited 10 minutes, the probability of waiting another 5 is the same as if you just started waiting.

3\. Normal (Gaussian) Distribution ($X \\sim \\mathcal{N}(\\mu, \\sigma^2)$)
...
$$F\_X(x) = P(X \\le x) = \\int\_{-\\infty}^x f\_X(t) \\, dt$$
...
$$p\_{X|Y}(x|y) = \\frac{p\_X(x) \\cdot p\_{Y|X}(y|x)}{p\_Y(y)}$$

**Analogy:**

* **Prior** $p\_X(x)$: What you believed before seeing data.
    
* **Likelihood** $p\_{Y|X}(y|x)$: How likely the data is given your belief.
    
* **Posterior** $p\_{X|Y}(x|y)$: Your updated belief after seeing the data.
...
<table><tbody><tr><td colspan="1" rowspan="1"><p><strong>Concept</strong></p></td><td colspan="1" rowspan="1"><p><strong>Discrete</strong></p></td><td colspan="1" rowspan="1"><p><strong>Continuous</strong></p></td></tr><tr><td colspan="1" rowspan="1"><p><strong>Distribution</strong></p></td><td colspan="1" rowspan="1"><p>PMF: $p_X(x)$</p></td><td colspan="1" rowspan="1"><p>PDF: $f_X(x)$</p></td></tr><tr><td colspan="1" rowspan="1"><p><strong>Expectation</strong></p></td><td colspan="1" rowspan="1"><p>$\sum x \cdot p_X(x)$</p></td><td colspan="1" rowspan="1"><p>$\int x \cdot f_X(x) \, dx$</p></td></tr><tr><td colspan="1" rowspan="1"><p><strong>Variance</strong></p></td><td colspan="1" rowspan="1"><p>$\sum (x-\mu)^2 \cdot p_X(x)$</p></td><td colspan="1" rowspan="1"><p>$\int (x-\mu)^2 \cdot f_X(x) \, dx$</p></td></tr><tr><td colspan="1" rowspan="1"><p><strong>Independence</strong></p></td><td colspan="1" rowspan="1"><p>$p_{X,Y} = p_X \cdot p_Y$</p></td><td colspan="1" rowspan="1"><p>$f_{X,Y} = f_X \cdot f_Y$</p></td></tr></tbody></table>
